...
export default function Dashboard() {
  ...
  // Reduced from 10 000 ms → 3 000 ms so Open price and P&L update every 3 s
  const { data: positions } = useGetPositions({ query: { refetchInterval: 3000 } });
      ...
      <div className="grid min-w-0 gap-4 md:grid-cols-7">
        <Card className="min-w-0 border-border md:col-span-5">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            {/* Pulsing dot indicates positions are refreshing every 3 s */}
          ...
          <CardContent className="min-w-0">
            {positions && positions.length > 0 ? (
                ...
                <Table className="min-w-[620px]">
                  <TableHeader>
                    <TableRow>
                      ...
                      {/* Open — the price at which the position was entered */}
                      <TableHead>Open</TableHead>
                      {/* Current — live bid (for BUY) or offer (for SELL) */}
                  ...
                  <TableBody>
                    {positions.map((pos) => {
                      // Show the live closing price relevant to the direction
                      const currentPrice = pos.direction === "BUY" ? pos.currentBid : pos.currentOffer;
                      return (
                        <TableRow key={pos.dealId}>
                          ...
                          {/* Open price — where the trade was entered */}
                          <TableCell className="font-mono text-muted-foreground">
                            {pos.openLevel > 0 ? formatNumber(pos.openLevel) : <span className="text-destructive/60">—</span>}
                          </TableCell>

                          {/* Current live price */}
                          <TableCell className="font-mono font-semibold text-foreground">
                            {currentPrice > 0 ? formatNumber(currentPrice) : <span className="text-muted-foreground">—</span>}
                          </TableCell>

                          {/* P&L — calculated on the server from live bid/offer vs open level.
                              cnProfitLoss returns text-primary (green) for profit > 0
                              and text-destructive (red) for profit < 0. */}
                          <TableCell
                            className={cn(
                          ...
                          <TableCell className="text-right">
                            <Button
                              ...
                              aria-label={`Close ${pos.market} position`}
                            >
              ...
              <div className="flex h-32 items-center justify-center rounded-sm border border-dashed border-border text-sm font-mono text-muted-foreground">
                NO_OPEN_POSITIONS
              </div>
            )}
          </CardContent>
