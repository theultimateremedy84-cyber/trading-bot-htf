...
export interface CapitalPosition {
  position: {
    ...
    direction: string;
    /**
     * Capital.com's REST API returns the opening price as `level` in the
     * actual JSON response. The field `openLevel` does NOT appear in the
     * live /positions payload — it was a misnamed type. Both names are kept
     * here so existing code keeps compiling while the route can safely read
     * whichever field is non-zero.
     */
    level?: number;
    openLevel?: number;
    openDate: string;
...
/**
 * Deal confirmation from Capital.com /confirms/{dealReference}.
 *
 * FIX: Expanded to capture `level` (the actual fill price Capital.com used)
 * and `size` (the filled size). Previously only `dealId` and `status` were
 * typed, so the actual fill price was silently discarded and the bot always
 * recorded the strategy's *calculated* entry price instead — causing a
 * persistent mismatch between the dashboard and Capital.com's own P&L.
 */
export interface DealConfirmation {
  dealId: string;
  status: string;
  /** Actual fill price used by Capital.com. Use this as entryPrice in the DB. */
  level?: number;
  /** Filled size (may differ from requested size on partial fills). */
  size?: number;
  profit?: number;
  reason?: string;
  epic?: string;
...
export class CapitalApiClient {
  ...
  async getDealConfirmation(dealReference: string): Promise<DealConfirmation> {
    await this.ensureSession();
    return this.request<DealConfirmation>(
      "GET",
