...
export interface CapitalMarket {
  ...
  marketStatus: string;
  scalingFactor?: number;
  /** Minimum deal size as reported by Capital.com dealingRules.minDealSize.
   *  This is account-specific and overrides any hardcoded fallback. */
  minDealSize?: number;
  /** Maximum deal size as reported by Capital.com dealingRules.maxDealSize. */
  maxDealSize?: number;
}
  ...
  async getSingleMarket(epic: string): Promise<CapitalMarket | null> {
    ...
    try {
      const data = await this.request<{
        instrument: CapitalMarket & {
          epic: string;
          name?: string;
          instrumentName?: string;
        };
        // NOTE: Capital.com returns `dealingRules` as a TOP-LEVEL sibling of
        // `instrument` — not nested inside it. Reading it from `instrument`
        // always yielded undefined, so the bot fell back to the hardcoded
        // 0.1 lot minimum and every forex order was rejected with
        // error.invalid.size.minvalue.
        dealingRules?: {
      ...
      const rules =
        data.dealingRules ??
        (data.instrument as unknown as { dealingRules?: { minDealSize?: { value?: number }; maxDealSize?: { value?: number } } }).dealingRules;

      const apiMinDealSize = rules?.minDealSize?.value;
      const apiMaxDealSize = rules?.maxDealSize?.value;

      return {
        epic: data.instrument.epic || epic,
        instrumentName: data.instrument.name || data.instrument.instrumentName || epic,
        ...
        minDealSize: typeof apiMinDealSize === "number" && apiMinDealSize > 0 ? apiMinDealSize : undefined,
        maxDealSize: typeof apiMaxDealSize === "number" && apiMaxDealSize > 0 ? apiMaxDealSize : undefined,
      };
    } catch {
      return null;
    }
