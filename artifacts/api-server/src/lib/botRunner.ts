...
async function scanMarkets() {
  ...
  try {
    ...
    // Raw P&L since this session started
    const rawSessionPnl = tradesRows
      .filter((t) => t.exitDate && new Date(t.exitDate) >= sessionStart && t.profit !== null)
      .reduce((sum, t) => sum + (t.profit ?? 0), 0);

    // Effective P&L — adjusted by any manual reset the user has applied
    const effectivePnl = rawSessionPnl - state.sessionPnlOffset;
    // ─────────────────────────────────────────────────────────────────────────

    // ── Daily profit target halt check ───────────────────────────────────────
    if (settings.haltOnDailyProfit && effectivePnl >= settings.dailyProfitTarget) {
      logger.info(
        { effectivePnl: effectivePnl.toFixed(2), target: settings.dailyProfitTarget },
