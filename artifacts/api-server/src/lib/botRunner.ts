...
async function scanMarkets() {
  ...
  try {
    ...
    try {
      ...
      // Force re-authentication next scan
    ...
    for (const epic of markets) {
      try {
        ...
        const [
          ...
          state.client.getCandles(epic, "DAY", 600),   // ~2 years of daily bars
        ...
        if (!signal) {
          logger.info(
            ...
            "No signal generated — HTF alignment missing, confidence below threshold, or no entry confluence (OB/FVG). Check Railway logs for detail."
          );
          continue;
        }
        ...
        logger.error({ err: errMsg, epic }, "Error scanning market");
      }
    }
