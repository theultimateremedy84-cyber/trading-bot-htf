 ...
 *            (in riskManager.ts getMinStopDistance). Capital.com demo tolerates
 *            stops as close as 5–7 pips; live accounts enforce a stricter
 *            minimum (~15–20 pips). The previous 0.1 % produced stops of ~7
 *            pips on AUDUSD at 0.70 which live rejected. 0.3 % gives ~21 pips.
 *   [Fix #12] Pre-flight price validation: immediately before createPosition(),
 *            verify stop is strictly below bid (BUY) or above offer (SELL) and
 *            target is strictly above offer (BUY) or below bid (SELL). If
 *            either check fails the trade is skipped with a clear warning log
  ...
  try {
    ...
    for (const epic of markets) {
      try {
        ...
        // Use Capital.com's dealingRules.minDealSize as the authoritative floor.
        // This is account-specific (demo vs live can differ), and instrument-
        // specific (AUDUSD live returned 1.0 minimum but our hardcoded value
        // was 0.1 — causing error.invalid.size.minvalue on every AUDUSD trade).
        // Fall back to getMinSizeForEpic() only when the API did not return it.
        const apiMin = marketData.minDealSize;
        const effectiveMinSize = apiMin !== undefined ? apiMin : getMinSizeForEpic(epic);
        const apiMax = marketData.maxDealSize;

        // Capital.com quotes forex deal size in UNITS of the base currency on
        // live accounts (minDealSize is typically 1000), not in 100k lots.
        // Detect that from the API minimum: when the minimum is >= 1 the size
        // field is already denominated in units, so the contract multiplier is 1.
        const sizeInUnits = apiMin !== undefined && apiMin >= 1;
        const contractMultiplier = sizeInUnits ? 1 : undefined;

        // Rounding step must match the venue: unit-denominated sizes are whole
        // numbers, lot-denominated sizes keep the instrument's decimals.
        const decimalPlaces = sizeInUnits ? 0 : getDecimalPlacesForEpic(epic);

        // Respect the API maximum when present; never let the hardcoded cap sit
        // below the exchange minimum (that is what silently produced sizes that
        // Capital.com rejected with error.invalid.size.minvalue).
        const hardcodedMax = getMaxSizeForEpic(epic);
        const effectiveMaxSize = Math.max(
          effectiveMinSize,
          apiMax !== undefined ? Math.min(apiMax, Math.max(hardcodedMax, sizeInUnits ? effectiveMinSize * 20 : hardcodedMax)) : (sizeInUnits ? effectiveMinSize * 20 : hardcodedMax)
        );

        logger.info(
          { epic, apiMinDealSize: apiMin, apiMaxDealSize: apiMax, effectiveMinSize, effectiveMaxSize, sizeInUnits, decimalPlaces },
          "Dealing rules resolved from Capital.com /markets/{epic}"
        );
        ...
        try {
          ...
          // the body and log it as a separate structured field so the full
          // errorCode is always visible in Railway structured logs.
          // ──────────────────────────────────────────────────────────────────
