  ...
  try {
    ...
    for (const epic of markets) {
      try {
        ...
        if (apiMin !== undefined && apiMin !== getMinSizeForEpic(epic)) {
          logger.info(
            { epic, apiMinDealSize: apiMin, hardcodedFallback: getMinSizeForEpic(epic) },
            "Using Capital.com API minDealSize (overrides hardcoded fallback)"
          );
        }
