    ...
    for (const epic of markets) {
      try {
        ...
        try {
          ...
          // FIX: Only record the trade in the database when Capital.com
          // explicitly confirms the deal was ACCEPTED. Previously the trade was
          // always inserted regardless of the confirmation status, which caused
          // rejected/pending orders to appear as real trades in the dashboard,
          // inflating the trade count and corrupting win-rate calculations.
          if (confirmation.status !== "ACCEPTED") {
            logger.warn(
              { epic, dealId: confirmation.dealId, status: confirmation.status },
          ...
          logger.info({ epic, dealId: confirmation.dealId, size: sizeResult.size, direction: signal.direction }, "✅ Trade executed and recorded successfully");
        } catch (execErr) {
