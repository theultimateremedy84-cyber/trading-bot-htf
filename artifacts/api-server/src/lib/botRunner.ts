  ...
  try {
    ...
    for (const epic of markets) {
      try {
        ...
        // with HTTP 400 error.invalid.stop.  This check recalculates the
        ...
        // Also enforce the instrument's minimum stop distance now so the
        // subsequent guard below doesn't need to run twice.
