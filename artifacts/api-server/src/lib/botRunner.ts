...
async function scanMarkets() {
  ...
  try {
    ...
    // Use the bot's start time as the session boundary instead of midnight UTC.
    // This means daily profit/loss targets reset every time the bot is manually
    // started, regardless of what time of day it is.
    ...
    // If session P&L has reached or exceeded the target, close all
    // open positions and stop the bot automatically.
    if (settings.haltOnDailyProfit && todayPnl >= settings.dailyProfitTarget) {
      logger.info(
        { todayPnl: todayPnl.toFixed(2), target: settings.dailyProfitTarget },
        "🎯 Daily profit target reached — closing all positions and halting bot"
      );
      // Close every open position on Capital.com
      try {
      ...
      // Stop the bot loop — this is the auto-halt
      await stopBot();
      return;
    }
    // ─────────────────────────────────────────────────────────────────────────
...
export async function startBot(): Promise<void> {
  ...
  state.running = true;
  state.startedAt = new Date();  // ← this timestamp is the session boundary for P&L targets
  state.error = null;
