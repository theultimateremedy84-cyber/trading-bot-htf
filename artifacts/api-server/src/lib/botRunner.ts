...
async function monitorPositions() {
  ...
  try {
    ...
    // simultaneous SL/TP hit or an API anomaly. Log for visibility but still
    // proceed — we verify each trade individually below.
    ...
    for (const trade of openTrades) {
      ...
      try {
        ...
        if (!pos) {
          // Position not on Capital.com — fetch current market price to
          // calculate the approximate exit P&L. If market data is unavailable
          // skip this trade and retry on the next cycle.
          ...
          const exitPrice = (market.bid + market.offer) / 2;
