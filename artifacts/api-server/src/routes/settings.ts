...
router.get("/settings", async (req, res) => {
  try {
    ...
    const effective = {
      ...
      capitalApiUrl:     process.env["CAPITAL_API_URL"]     || row.capitalApiUrl,
      // password never exposed — just note env var presence
    };
...
router.put("/settings", async (req, res) => {
  try {
    ...
    // unless the caller explicitly provided a custom URL at the same time.
    if (body.isDemo !== undefined) {
      updateData.isDemo = body.isDemo;
      if (body.capitalApiUrl === undefined) {
        // Auto-switch URL based on the demo flag
