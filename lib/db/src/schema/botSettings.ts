...
export const botSettingsTable = pgTable("bot_settings", {
  ...
  // Daily profit target halt — close all positions and stop bot when today's P&L hits this
  dailyProfitTarget: real("daily_profit_target").notNull().default(40.0),
  ...
  // FIX #4: lowered from 65 → 55.  65 required kill-zone bonus + 4+ signals
  // simultaneously just to reach threshold outside the 7-hour kill-zone window.
  // 55 is achievable with 2/3 HTF + daily alignment + two entry confluences —
  // a realistic but still meaningful bar.
